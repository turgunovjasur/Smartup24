...
def run_industry(page: Page, code) -> None:
    ...
    with allure.step("Отрасль guruhini ochish va Подтипы ga o'tish"):
        m.click_grid_row("Отрасль")
        m.click_button("Подтипы")
        m.expect_heading("Отрасль")

    with allure.step("Создать: yangi Отрасль подтипи formasi ochish"):
        m.open_create()
...
@allure.story("Характеристика товаров — Отрасль")
@allure.title("Отрасль учун yangi подтип yaratish")
